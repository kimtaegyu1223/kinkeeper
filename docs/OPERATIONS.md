...
- **`SCHEDULE_HORIZON_DAYS` 값** — 코드 기본값은 365, 옛 문서·일부 예시는 60이었음. 최종 운영값을 확정해 `.env`에 고정할지. **주인 결정 대기.**
- **음력 윤달·2/30 스키마** — 해당 구성원이 실제로 있으면 `month`/`day`/`is_leap` 컬럼 전환이 필요(현재는 입력 단계에서 거부). **주인 결정 대기.**
