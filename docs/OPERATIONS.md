...
`/healthz` 실패(웹 다운으로 연결 거부 또는 DB 장애로 503)를 감지하면 **관리자
개인 DM**(`ADMIN_CHAT_ID`)으로 경보를 보냅니다(가족방에는 보내지 않음 — 미설정 시
`GROUP_CHAT_ID` 폴백). 연속 장애 스팸을 막기 위해 **같은 장애당 1시간에 1회**만
경보하고(상태파일 쿨다운), 정상으로 돌아오면 **'복구됨' 1회**를 보냅니다. `.env`의
`TELEGRAM_BOT_TOKEN`/`ADMIN_CHAT_ID`를 사용합니다. 관리자가 봇에게 `/start`를
보낸 적이 있어야 DM 수신이 가능합니다.
...
**아직 경보하지 않은 새 실패가 생겼을 때만** 관리자 개인 DM(`ADMIN_CHAT_ID`,
미설정 시 `GROUP_CHAT_ID` 폴백)으로 1건 경보합니다.
