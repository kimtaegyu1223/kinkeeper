...
3. `_purge_old_notifications()` — 90일(`_RETENTION`) 지난 종료 상태(sent/failed/cancelled)
   행을 물리 삭제
...
**상태 기반 제너레이터** (`health_check.py`): DB의 현재 상태(검진 기록·주기)를 직접
순회해 알림을 계산하고 `source_key` upsert로 멱등하게 유지합니다. 이번 재빌드가
원하지 않게 된 `hc:%` pending은 `_cancel_stale_*` 함수가 `cancelled`로 바꿉니다.
...
- `upsert_notification_by_key()` — source_key 알림. `ON CONFLICT ... DO UPDATE`로
  ...
  있으므로). rule 없는 모든 알림(건강검진)이 이 한 규칙을 공유합니다.
...
2. `send_message(chat_id, message)` 호출 후, **아직 `pending`인 행만** 조건부 UPDATE로
...
| 테이블 | 역할 | 메모 |
|---|---|---|
| `family_members` | 구성원 마스터 | `telegram_user_id` nullable(계정 없어도 등록·그룹 알림만 수신), `gender` M/F/None(None=미설정→성별 지정 항목도 포함), `birthday_solar`/`birthday_lunar` 독립 nullable |
...
건강검진은 `rule_id`가 없으므로 CASCADE 삭제가 불가능합니다. 대신 `source_key`로 멱등
upsert하고, 대상에서 빠진 묵은 알림은 `_cancel_stale_*`가 취소합니다. 현재 코드가
실제로 만드는 키:
...
| **source_key dedup** | 건강검진 알림은 rule_id가 없어 CASCADE 불가. source_key upsert로 멱등 재빌드 |
